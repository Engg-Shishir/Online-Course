...
function SendContact(contact_name,contact_mobile,contact_email,contact_msg) {
    ...
    if(contact_name.length==0){
        ...
        axios.post('/contactSend',{contact_name:contact_name,contact_mobile:contact_mobile,contact_email:contact_email,contact_msg: contact_msg,
        })
        .then(function (response) {
                if(response.status==200){
                    if(response.data==1){
                        ...
                        toastr.success('Send Success');// Show toaster alert Success 
                        setTimeout(function () {
                            $('#contactSendBtnId').html('পাঠিয়ে দিন');
                        },3000)
